...
| R-19 | Done | P1 | backend/frontend/test/docs | Feed refresh diagnostics and add preflight | 每个 feed 展示 lastFetchedAt、lastSuccessAt、lastError、健康状态和手动重试入口；添加前可从网站首页发现候选 RSS / Atom，预览最近文章，识别重复订阅；测试和 API 文档覆盖。 |
| R-20 | Done | P1 | docs | User documentation v0 | README 覆盖安装、首次设置、OPML、provider、备份、升级、常见问题；非开发者可按步骤完成部署。 |
| R-21 | Done | P1 | frontend/backend/test/docs | Unread debt cleanup | latest/recommended 支持全部、24h 前、7d 前、30d 前清账，search 支持当前结果清账；清账是 Reader Command，不是 Behavior Event，不作为推荐正反馈；只支持批量已读，不支持批量删除/收藏/不感兴趣。 |
...
## M4: 0.2 Developer Platform And Plugin System

目标：从 `0.2.0` 开始，把面向开发者的扩展能力文档化，并交付可安装、可升级、可随 Docker 升级保留的插件系统。

当前状态：Todo。插件系统设计已形成，但实现、开发者文档和首个官方插件仍待拆分落地。

Developer documentation rules:

- `0.2.0` 起新增独立开发者文档，不混在 README 用户文档里。
- 开发者文档只维护简体中文和英文，不要求日文版。
- 插件开发文档是开发者文档的子单元，同时必须是独立文件，方便插件作者直接阅读和链接。
- 建议文件形态：
  - `docs/developer-guide.zh-CN.md`
  - `docs/developer-guide.en-US.md`
  - `docs/plugin-development.zh-CN.md`
  - `docs/plugin-development.en-US.md`

| ID | Status | Priority | Type | Task | Acceptance Criteria |
| --- | --- | --- | --- | --- | --- |
| M4-01 | Todo | P1 | docs | Developer documentation foundation | 新增中/英双语开发者文档，覆盖本地开发、仓库结构、API/DB 约定、测试命令、发布前检查；不要求日文版。 |
| M4-02 | Todo | P1 | docs | Plugin development documentation | 新增中/英双语独立插件开发文档，作为开发者文档子单元；覆盖 manifest、capabilities、UI slots、Hooks、tasks、安装包、分发、更新、Docker 升级持久化和官方插件边界。 |
| M4-03 | Todo | P1 | frontend/backend/infra/test/docs | Plugin manager and registry foundation | `Settings -> Plugins` 可列出官方/第三方插件，第三方插件安装在 `/data/plugins/installed`，官方插件从 `/app/plugins/official` 加载，Docker 升级后兼容插件自动恢复，不兼容插件保留数据并暂停。 |
| M4-04 | Todo | P1 | backend/test/docs | Plugin hooks and tasks foundation | 插件可注册首批 Hook 和后台任务；插件 job type 使用 `plugin:<pluginId>:<taskId>`；不兼容插件的 pending jobs 被暂停而不是误报 `No handler registered`。 |
| M4-05 | Todo | P1 | frontend/backend/algorithm/test/docs | Daily Brief official plugin | `app.dibao.daily-brief` 作为官方插件随 release 分发，可在设置中启用；支持按日生成简报、手动前台生成、基于 24h 个性化排名并按兴趣簇多样化。 |
...
3. `M4-01 / M4-02`: Developer docs and standalone plugin development docs for `0.2.0`。
4. `M4-03 / M4-04`: Plugin manager, persistence, hooks and tasks foundation。
5. `M4-05`: Daily Brief official plugin。
6. `M3-11`: Desktop/mobile app packaging research。
