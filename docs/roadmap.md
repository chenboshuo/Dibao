...
- 已完成基础工程、数据库、单用户 setup/auth/session、首次设置向导、RSS 入库、OPML API、Web OPML UI、feed folder 展示筛选、feed 管理 UI、后台刷新 job、文章保留清理、文章列表分页、基础 Web 阅读器、文章动作、baseline ranking、推荐解释和 i18n foundation。
- 尚未完成让普通自托管用户可用的关键闭环：Docker 打包、设置页、embedding provider、embedding job pipeline、sqlite-vec 生产集成、Profile Algorithm v0、推荐排序 v1、E2E 测试。
...
当前状态：In Progress。单用户 auth、首次设置向导、feed 管理闭环、后台刷新和数据保留 foundation 已完成，设置页、Docker、E2E 和用户文档仍待完成。
...
1. `M2-06 / R-13`: Settings page v0。
2. `M3-01 / M3-02`: Embedding provider settings and adapter interface。
3. `M3-03 / M3-04`: Embedding queue and sqlite-vec production adapter。
4. `M3-05 / M3-06`: Profile Algorithm v0 and Ranking v1。
5. `M2-07 / M2-08 / M2-12`: Docker, E2E smoke, user docs。
